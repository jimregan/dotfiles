...
do
	...
	links=$(lynx -useragent="$user_agent" -dump "$i"|grep -v 'http://www.jjgirls.com/japaneses/'|grep -v '^\['|grep -i '\(jpe*g$\|jpg?cb=1$\|png$\|jpg_[0-9]*$\)'|sed -e 's/?cb=1$//'|awk '{print $2}'|sed -e 's#http://promo.elitetvonline.com/galleries/picview.asp?pic=#http://promo.elitetvonline.com/galleries/#g;s/\(s[0-9]*0\)-h/\1/'|sed -e 's/\[[0-9]*\]http/http/')
	for l in $links
	do
