...
sub genBlank($) {
	my ($inname) = shift;
	$inname =~ s/ //g;
	return "http://imgmeta.sourceforge.net/0.1/UFM#" . uri_escape_utf8($inname);
}
...
for my $model (@{${$result}{'model'}}) {
	my $id = genBlank($model->{'name'});
	my $url = get_url($model->{'url'});
	my $img = $base . $model->{'pic'};

	print "$id <http://www.w3.org/1999/02/22-rdf-syntax-ns#> <http://xmlns.com/foaf/0.1/Person> .\n";
	print "$id <http://www.w3.org/1999/02/22-rdf-syntax-ns#> <http://dbpedia.org/ontology/Model> .\n";
	print "$id <http://xmlns.com/foaf/0.1/name> \"$model->{'name'}\" .\n";
