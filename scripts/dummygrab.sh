...
do
	...
	links=$(lynx -useragent="$user_agent" -dump "$i"|grep -i 'jpe*g$'|awk '{print $2}'|sed -e 's#http://promo.elitetvonline.com/galleries/picview.asp?pic=#http://promo.elitetvonline.com/galleries/#g'|sed -e 's/\[[0-9]*\]http/http/')
	for l in $links
	do
