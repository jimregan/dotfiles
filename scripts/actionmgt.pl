...
sub get_models($) {
	...
	my $tpl = << "__END__";
<form id="form1" name="form1" method="post" action=""><td width="18%" align="right" valign="middle"><select name="model" id="model"[% ... %]>[% ... %]<option value="" selected="selected">MODEL NAME</option>[% ... %][% FOREACH model %]<option value="[% id %]">[% name %]</option>[% END %][% ... %]</select></td></form>
__END__
